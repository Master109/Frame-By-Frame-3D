...
public class WavesAnimation : MonoBehaviour, IUpdatable
{
	...
	public virtual void DoUpdate ()
	{
		...
		if (wavesAudioTrs != null)
			wavesAudioTrs.position = GameManager.GetSingleton<OVRCameraRig>().centerEyeAnchor.position + (new Vector2(material.mainTexture.width, material.mainTexture.height).Multiply(material.mainTextureOffset) * audioMovmentMultiplier).XYToXZ();
	}
